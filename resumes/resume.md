...
- データ整合性：冪等性、状態遷移、境界条件、再実行可能性、説明可能性
- DB：MySQL（トランザクション、ロック、運用要件を前提とした設計）
- 非同期処理：成功と完了の分離、ジョブ整合性、再実行戦略
- ワークフロー：Digdag（確定ルール、リトライ、通知設計）
- 分析基盤：AWS Athenaをベースとした DWH 周りの仕組み
...
- 大量取り込みと誤投入防止がトレードオフになるため、Validation と投入を分離し、投入条件を厳格化した。
