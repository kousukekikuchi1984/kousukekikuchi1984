...
ミッションクリティカル領域で、誤ったデータが「正として確定しない」ための設計・判断を担うバックエンドエンジニアです。会計データやポイント残高のような不可逆データを主戦場に、冪等性／状態遷移／再実行戦略／監査可能性を含む整合性設計を行ってきました。会員700万人規模のポイントプラットフォームでは、会計出力・外部カード連携・運用者向け処理まで横断し、短期的な性能最適化よりも長期運用での再現性・説明可能性・運用収束を優先して意思決定・実装してきました。現在は株式会社 NTT-ME にて、英国 Tractable 社から譲渡された AI 建物レジリエンス事業のシステム掌握・運用継続・改善に取り組んでいます。
...
- 期限制約下でも、冪等性／再実行性／監査性を落とさない実装・リリース設計
- レビュー待ちがボトルネックになる速度感で改善を回しつつ、境界（非同期・デプロイ・部分失敗・トランザクション境界）で壊れないことを優先

# 経歴
## Software Engineer（株式会社 NTT-ME）

2026.06-現在 在籍

### 所属

- 社会インフラデザイン部 地域あんしん推進部門 防災担当 AI建物レジリエンス事業
- 英語表記：NTT-ME Corporation Social Infrastructure Design Department AI Property REsilience Business

### 概要

- 英国 Tractable 社から譲渡された AI 建物レジリエンス事業に、ソフトウェアエンジニアとして参画
- 事業譲渡後のフェーズとして、既存システムの構造・運用・技術的前提の把握を推進
- 防災・建物レジリエンス領域におけるサービス継続性を前提に、システムの掌握と改善余地の特定を進めている

### 役割概要

- 既存システムのアーキテクチャ、コードベース、データフロー、運用フローの理解
- 譲渡後の運用継続に必要な技術的リスク・属人化ポイント・改善課題の洗い出し
- 事業・運用・技術の前提を整理し、今後の開発・保守に必要な判断材料を整備
...
- 大手スポーツ量販店向けポイントプラットフォーム（会員 700万人、将来 1000万人規模 を想定）
- ポイント残高・会計出力・外部カード連携など、誤りが確定すると修正困難な領域を担当
- ユーザー向けAPI〜管理画面/運用者向け処理まで横断し、データ整合性・運用性を含めて設計
- 冪等性/再実行、ロック順序、会計確定ルール等の設計規約を言語化してレビュー観点として定着させ、重大障害・誤計上に直結するリスクを設計是正で事前に潰し込み、運用で説明可能な形に収束させた
...
- 30万件/1時間SLAの外部連携を、ユーザー単位ロック＋検証分離＋冪等再実行で設計し、実運用で約6分の取り込みに収束

##### 課題
クレジットカード会社とのデータ連携において、30万件のデータを1時間以内に Web アプリケーション内に安全に取り込むという制約があった。
...
- 実行が求められた primitive 化などの局所最適は、Python では cache locality があたらないため効果が低いどころか復旧ロジックの二重化・理解困難化を招くという仮説を立てた
- 調査の結果、ボトルネックがアプリケーションではなくMySQL の commit 時 fsync に待ちがあることを特定
...
- 本番メモリリークの原因を永続キャッシュによるEngine参照に特定し、ライフサイクル設計（保持しない／破棄可能化）へ改修して安定稼働を回復した。
