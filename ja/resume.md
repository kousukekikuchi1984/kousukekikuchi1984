...
ミッションクリティカル領域で、誤ったデータが「正として確定しない」ための設計・判断を担うバックエンドエンジニアです。会計データやポイント残高のような不可逆データを主戦場に、冪等性／状態遷移／再実行戦略／監査可能性を含む整合性設計を行ってきました。会員700万人規模のポイントプラットフォームでは、会計出力・外部カード連携・運用者向け処理まで横断し、短期的な性能最適化よりも長期運用での再現性・説明可能性・運用収束を優先して意思決定・実装しています。
