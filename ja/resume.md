...
ミッションクリティカルな領域において、誤ったデータが正として確定しない構造を設計・判断するバックエンドエンジニアとして業務に従事してきました。会計データやポイントシステムなど、「一度確定すると後から戻せないデータ」を主戦場とし、実装だけでなく設計段階での判断に責任を持つ役割を担っています。特に短期的な性能改善よりも、長期運用におけるデータ整合性・再現性・説明可能性を重視しています。スプリントではストーリーポイント1〜2の開発を午前中に完了させる実装速度を保ちながら、期限が厳しいアプリ向けバックエンド案件でもデリバリを優先度高く推進しています。

## 技術領域

| 項目名   | 内容                                                 |
| -------- | ---------------------------------------------------- |
| Backend   | Python（主戦場: 内部実装理解、メモリプロファイリング, 非同期処理),  Rust(限定的), Java（限定的）, Go(限定的) |
| Frontend  | TypeScript(業務利用), Flutter(限定的) |
| Database   | MySQL(主戦場: 性能・運用), DuckDB(業務利用), AWS Athena(業務利用: 分析基盤) |
| Infra/Data   | AWS(ECS, EKS, Lambda, CloudFront, CloudWatch, EventBridge, StepFunction), Digdag（コード解析）、 Embulk(コード解析) |
| Others | Linux Kernel, HULFT（レガシー理解） |
...
### 短期デリバリと高信頼運用の両立
- スプリント計画ではストーリーポイント1〜2の改善を午前中に仕上げ、実装完了よりレビュー待ちがボトルネックになるペースを維持
- 納期が固定されたバックエンド案件では、要件の安全ラインを守りつつ検証・リリースを高速に回し、時間制約と品質を両立
