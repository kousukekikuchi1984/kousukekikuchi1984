...
#### クレジットカード会社とのファイル連携基盤と更新（HULFT／運用設計）

WIP 

##### 課題
クレジットカード会社とのデータ連携にあたり、HULFT を用いたファイル受信基盤の構築が必要だった。
通信要件・監視・障害時対応・切替手順が未整理な状態であり、切替失敗時には業務影響が大きいリスクを抱えていた。

##### 判断・行動
- 受信側として HULFT の設定・疎通確認を行い、CloudWatch Logs Agent によるログ監視を構築
- 障害時に「どのログを見れば切り分けできるか」を明確にし、運用時の確認観点を整理
- 当初は段階的切替による安全な移行を検討していたが、先方より切替に数百万円規模のコストが発生する旨が提示された
- コスト・影響範囲・失敗時の復旧可能性を整理した上で、リスクを許容し一括切替を行う判断を実施
- 切替当日は重点監視体制を敷き、問題発生時に即時対応できる状態を整えた

##### 結果
- 致命的な障害はなく切替を完了し、連携基盤を安定稼働させることに成功
- 対外連携における技術的判断とリスクテイクを含む意思決定を担い、運用可能な状態を確立
